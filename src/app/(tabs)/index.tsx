...
export default function TodayRoute() {
  ...
  return <FocusedScene accessibilityElementsHidden={isFocused ? false : true} focused={isFocused} importantForAccessibility={isFocused ? 'auto' : 'no-hide-descendants'}><TodayReferenceScreen onCreatePlan={() => router.push('/setup' as Href)} onOpenCycle={() => router.push('/cycle-completion' as Href)} activeReadiness={activeReadiness} onOpenReview={() => router.push('/weekly-review' as Href)} recommendations={<SessionReviewPanel key={`${isFocused}`} reviews={sessionReviews} onChanged={onReviewChanged} />} savedReadiness={persistedReadiness} initialReadinessInput={persistedReadiness?.input ?? null} readinessGate={ReadinessGate} state={state} onOpenSettings={() => router.push('/settings')} onApplyReadiness={async (input) => {
