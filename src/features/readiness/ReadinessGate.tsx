...
function ReadinessChoice({ busy, label, copy, decision, selected, onSelect, abdominalRestrictionActive }: { busy: boolean; label: string; copy: string; decision: SafetyInput; selected: boolean; onSelect: (input: SafetyInput) => Promise<void>; abdominalRestrictionActive: boolean }) {
  return <ChoiceControl busy={busy} accessibilityLabel={label} label={copy} selected={selected} onPress={() => { void onSelect({ ...decision, ...(abdominalRestrictionActive ? { abdominalRestrictionActive: true } : {}) }); }} />;
}
    ...
    <Modal animationType={reducedMotion ? 'none' : 'slide'} onRequestClose={close} role="dialog" transparent visible={visible}>
      <View ref={overlayRef} style={[styles.overlay, { backgroundColor: theme.overlay }]}>
        <View accessible accessibilityLabel="Preparación de hoy" accessibilityViewIsModal role="dialog" testID="readiness-focused-surface" style={[styles.sheet, { backgroundColor: theme.surface }]}>
          <AppMasthead inset={0} role="task" command={<IconButton accessibilityLabel="Cerrar Preparación de hoy" icon={X} onPress={close} />} context="Control local antes de entrenar" title="PREPARACIÓN DE HOY" />
        ...
        {!locked && ([
          ...
          return <View key={label} style={tone === 'caution' ? styles.caution : tone === 'danger' ? styles.danger : undefined}><ReadinessChoice busy={busy} label={label} copy={copy} decision={decision} selected={checked} onSelect={persist} abdominalRestrictionActive={Boolean(input?.abdominalRestrictionActive)} /></View>;
        })}
