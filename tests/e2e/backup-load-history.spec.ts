...
test('restores mixed load history and an active workout, rejecting invalid corrections without data loss', async ({page,context},info)=>{
 ...
 await expect(app.getByLabel('Documento JSON de respaldo',{exact:true})).toHaveCount(0);
 const downloadEvent=app.waitForEvent('download');
 await app.getByRole('button',{name:'Guardar archivo cifrado',exact:true}).click();
 const download=await downloadEvent;
 expect(download.suggestedFilename()).toMatch(/^strength-rebuild-.*\.srb$/);
 const backupPath=info.outputPath(download.suggestedFilename());
 await download.saveAs(backupPath);
 expect(readFileSync(backupPath,'utf8')).toMatch(/^SRB2\./);
 expect(readFileSync(backupPath,'utf8')).not.toContain('workout_session');
 const pick=async (file: string | {name:string;mimeType:string;buffer:Buffer})=>{
   const chooserEvent=app.waitForEvent('filechooser');
   await app.getByRole('button',{name:'Importar archivo de respaldo',exact:true}).click();
   await (await chooserEvent).setFiles(file);
 };
 await app.getByLabel('Contraseña portátil del respaldo',{exact:true}).fill('wrong-synthetic-password');
 await pick(backupPath);
 await expect(app.getByText(/contraseña es incorrecta|respaldo fue alterado/i)).toBeVisible();
 ...
 await app.getByLabel('Contraseña portátil del respaldo',{exact:true}).fill('synthetic-load-restore');
 await pick({name:'corrupt.srb',mimeType:'application/octet-stream',buffer:Buffer.from('SRB2.corrupt')});
 await expect(app.getByRole('button',{name:'Confirmar restauración del respaldo',exact:true})).toHaveCount(0);
 await expect(app.getByRole('button',{name:'Importar archivo de respaldo',exact:true})).toBeEnabled();
 expect(await readPersistence(app,info)).toEqual(before);
 await pick({name:'future.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify({version:999,tables:{}}))});
 await expect(app.getByRole('button',{name:'Importar archivo de respaldo',exact:true})).toBeEnabled();
 await expect(app.getByRole('button',{name:'Confirmar restauración del respaldo',exact:true})).toHaveCount(0);
 expect(await readPersistence(app,info)).toEqual(before);
 await pick(backupPath);
 await expect(app.getByRole('button',{name:'Confirmar restauración del respaldo',exact:true})).toBeVisible();
 await app.screenshot({path:info.outputPath('file-preview.png'),fullPage:true});
 expect(await readPersistence(app,info)).toEqual(before);
 ...
 await app.goto('/backup');
 await app.getByRole('button', { name: 'Mostrar importación avanzada', exact: true }).click();
