...
for(const choice of ['ACCEPTED','KEPT','REJECTED']) test(`verified weekly targets: ${choice} preserves originals and opens the next workout`,async({page,context},info)=>{
 ...
 await expect(app.getByText('1 × 6 → 1 × 7 repeticiones · 40 → 40 kg',{exact:true})).toHaveCount(6);
 ...
 await app.getByLabel('Dolor de 0 a 2, estable',{exact:true}).click();
 await app.getByRole('button',{name:'Confirmar preparación',exact:true}).click();
 await expect(app.getByLabel('Repeticiones de la serie 1',{exact:true})).toHaveValue(choice==='ACCEPTED'?'7':'6');
 ...
 expect(JSON.parse(String(active.prescribed_snapshot_json)).exercises.map((e:{target:{reps:{min:number}}})=>e.target.reps.min)).toEqual(choice==='ACCEPTED'?[7,7]:[6,6]);
