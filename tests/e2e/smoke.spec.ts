...
test('production plan and completed set survive reopening', async ({ page, context }, info) => {
  ...
  await expect.poll(async () => (await readPersistence(page, info)).sets, { message: 'Canonical completed-set values must persist' }).toEqual([
    { load: '', reps: '', notes: 'Synthetic persistence smoke', disposition: 'COMPLETED' },
  ]);
  ...
  expect(draft.exercises[0].sets[0]).toMatchObject({ completed: true, disposition: 'COMPLETED', seconds: '8', reps: '', notes: 'Synthetic persistence smoke' });
  ...
  await expect(reopened.getByLabel('Carga de la serie 1', { exact: true })).toHaveCount(0);
  await expect(reopened.getByLabel('Segundos de la serie 1', { exact: true })).toHaveValue('8');
