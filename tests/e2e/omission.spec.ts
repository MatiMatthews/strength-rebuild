...
test('omission drafts preserve data until confirmation and survive reopen exactly', async ({ page, context }, info) => {
  ...
  await (await openSetNotes(page, 1)).fill('Omission preservation');
  const snapshot = async (p = page) => (await readPersistence(p, info)).workouts;
  await expect.poll(async () => JSON.parse(String((await snapshot())[0]?.actual_snapshot_json)).exercises[2].sets[0].notes).toBe('Omission preservation');
  ...
  await expect.poll(async () => JSON.parse(String((await snapshot())[0]?.actual_snapshot_json)).exercises[2].sets[0].skipReason).toBe('Equipo no disponible');
  ...
  originalDraft.exercises[2].sets[0] = { ...originalDraft.exercises[2].sets[0], completed: false, skipped: true, disposition: 'SKIPPED', skipReason: 'Equipo no disponible' };
  ...
  await expect.poll(async () => JSON.parse(String((await snapshot(reopened))[0]?.actual_snapshot_json)).exercises[2].sets[0].skipReason).toBe('Motivo corregido');
  const edited = JSON.parse(String((await snapshot(reopened))[0]?.actual_snapshot_json));
  confirmedDraft.exercises[2].sets[0].skipReason = 'Motivo corregido';
